...
string applySBoxes(string input){
	string output = "" , rowString="" ; 
	for(int i =0 , sboxIndex =0  ;i < input.length() ; i+=6 , sboxIndex++){
		string temp = input.substr(i , 6) ; 
		rowString = temp[0] ; rowString+=temp[5] ; 
		int row = bitset<2>(rowString).to_ulong() ; 
		int col = bitset<4>(temp.substr(1,4)).to_ulong() ; 
		output+= bitset<4>( sbox[sboxIndex][row][col]).to_string() ; 
	}
	cout<<"Sbox output = " << output <<endl;  
	return output ; 
}

string applyPermuteTable(string input){
	string output = "" ; 
	for(int i =0 ;i< 32 ; i++) output+= input[permute[i]-1] ; 
	return output ; 
}
